...
func (repository *UserRepository) List(ctx context.Context) ([]models.User, error) {
	rows, err := repository.pool.Query(ctx, `SELECT id::text, email, created_at, updated_at FROM users ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (repository *UserRepository) Create(ctx context.Context, email string) (models.User, error) {
	var user models.User
	err := repository.pool.QueryRow(ctx, `INSERT INTO users (email) VALUES ($1) RETURNING id::text, email, created_at, updated_at`, email).Scan(&user.ID, &user.Email, &user.CreatedAt, &user.UpdatedAt)
	return user, err
}
