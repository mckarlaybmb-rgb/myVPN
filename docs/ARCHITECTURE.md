...
- `internal/jobs`: durable queue states, claiming, completion, failure, and enqueue operations. No persistent worker is started yet.
...
Xray HandlerService and StatsService integration, persistent workers, Prometheus metrics, payment, Telegram, dashboard, and multi-node orchestration are planned and are not implemented in this phase.
